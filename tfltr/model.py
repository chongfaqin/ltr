...
class BaseRankModel(object):
    ...
    def _init_tf_vars(self):
        with tf.name_scope(self.model_name):
            ...
            self.learning_rate = tf.train.exponential_decay(self.params["init_lr"], self.global_step,self.params["decay_steps"], self.params["decay_rate"])
    ...
    def _score_fn_inner(self, x, reuse=False):
        ...
        out = dense_block(x, hidden_units=hidden_units, dropouts=dropouts, densenet=False, reuse=reuse,training=self.training, seed=self.params["random_seed"])
        # score,output no activation
        score = tf.layers.dense(out, 1, activation=None,kernel_initializer=tf.glorot_uniform_initializer(seed=self.params["random_seed"]))
    ...
    def _init_session(self):
        config = tf.ConfigProto(log_device_placement=True,allow_soft_placement=True,inter_op_parallelism_threads=0,intra_op_parallelism_threads=0)
    ...
    def save_session(self):
        ...
        builder = tf.saved_model.builder.SavedModelBuilder("model/"+self.model_name+"/")
    ...
    def fit(self, X, validation_data):
        ...
        if validation_data is not None:
            ...
            self.logger.info("[epoch-{}, batch-{}] -- Train Loss: {:5f} NDCG: {:5f} ({:5f}) ERR: {:5f}  -- Valid Loss: {:5f} NDCG: {:5f} ({:5f}) ERR: {:5f} -- {:5f} s".format(
            ...
            self.logger.info("[epoch-{}, batch-{}] -- Train Loss: {:5f} NDCG: {:5f} ({:5f}) ERR: {:5f} -- {:5f} s".format(
                    0, 0, loss_mean_train, ndcg_mean_train, ndcg_all_mean_train, err_mean_train,
                    time.time() - start_time))
...
class LogisticRegression(DNN):
    ...
    def _score_fn_inner(self, x, reuse=False):
        score = tf.layers.dense(x, 1, activation=None, kernel_initializer=tf.glorot_uniform_initializer(seed=self.params["random_seed"]))
        return score
...
class RankNet(BaseRankModel):
    ...
    def _build_unfactorized_model(self):
        ...
        # self.logger.info(np.shape(s_i_minus_s_j))

        #compute real score
        S_ij = (self.label - tf.transpose(self.label))*100
        ...
        # true is 1,false is 0，1 is same query.
        mask1 = tf.cast(mask1, tf.float32)

        # exclude the pair of sample and itself
        n = tf.shape(self.feature)[0]
        # get diagonal of matrix is 0,this location is itself
        mask2 = tf.ones([n, n]) - tf.diag(tf.ones([n]))
        mask = mask1 * mask2
        # comput pairs count,element num of one in matrix
